...
void AnimationPanel::update(float dt)
{
    ...
    ImGui::BeginChild("Drop Zone", {ImGui::GetContentRegionAvail().x * 0.8f, ImGui::GetContentRegionAvail().y}, true);
    {
    ...
    ImGui::EndChild(); 
    ImGui::SameLine();
    ImGui::BeginChild("Preview Zone",ImGui::GetContentRegionAvail(), true);
    {
        sf::Sprite sprite(*static_cast<Editor*>(m_editor)->getElement<BrushPanel>("BrushPanel")->getBrush().texture);
        if(isPlaying){
